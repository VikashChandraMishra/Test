...
const Application = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem('authToken')) {
      ...
      const fetchData = async () => {
        const response = await fetch('http://3.111.49.143:5000/api/applicant/saved-form-data', {
          method: 'GET',
  ...
  const handleSave = async (e) => {
    ...
    const response = await fetch('http://3.111.49.143:5000/api/applicant/save-form', {
      method: 'POST',
